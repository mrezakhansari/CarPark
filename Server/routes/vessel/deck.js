...
router.post("/getListOfCntrsForStowage", auth, async (req, res) => {
    try {
        const db = sworm.db(setting.db.sqlConfig[`${req.user.area}`]);
        //console.log(req.body)
        var result = await db.query(queries.VESSEL.DECK.getListOfCntrsForStowage, {
            voyageId: req.body.voyageId
        });
        console.log(result)
        return SendResponse(req, res, result, result && result.length > 0);
    } catch (error) {
        return SendResponse(req, res, 'getListOfCntrsForStowage', false, 500);
    }
});
